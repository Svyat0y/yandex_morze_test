const input = '---.. T---.. -.... T..---   R--------.. ...-- E----- ----.   R--------.. R....------ E.--.- .----   R......---- -....' +
	' .---- -....'
const numbers = '8268 9309 9281 3616'
const morseNumbers = [
	'-----', // 0
	'.----', // 1
	'..---', // 2
...
const getNumberByMorse = morse => morseNumbers.indexOf(morse)

const decodeGroup = group => group.map(c => {
	let decoded = getNumberByMorse(c)

	if ( [ 'T', 'E', 'R' ].indexOf(c[0]) >= 0 ) {
...
console.log(result === numbers ? 'decoded' : 'error')